...
function Datepicker() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		keyModifier: 'ctrlKey', // The key modifier used for navigating, can be shiftKey, ctrlKey, altKey, metaKey or null
        highlightCurrentDay: true // True to highlght current day when switching months/years
	};
...
$.extend(Datepicker.prototype, {
	...
	_connectDatepicker: function(target, inst) {
		...
		if (showOn == 'button' || showOn == 'both') { // pop-up date picker when button clicked
			...
			var trigger = $(this._get(inst, 'buttonImageOnly') ? 
				$('<span/>').addClass(this._triggerClass).
	...
	_doKeyDown: function(e) {
		...
		var keyModifier = $.datepicker._get(inst, 'keyModifier');
		var handled = true;
		if ($.datepicker._datepickerShowing)
			switch (e.keyCode) {
				...
				case 33: $.datepicker._adjustDate(e.target, (!keyModifier || e[keyModifier] ?
							-$.datepicker._get(inst, 'stepBigMonths') :
							-$.datepicker._get(inst, 'stepMonths')), 'M');
						break; // previous month/year on page up/+ ctrl
				case 34: $.datepicker._adjustDate(e.target, (!keyModifier || e[keyModifier] ?
							+$.datepicker._get(inst, 'stepBigMonths') :
							+$.datepicker._get(inst, 'stepMonths')), 'M');
						break; // next month/year on page down/+ ctrl
				case 35: if (!keyModifier || e[keyModifier]) $.datepicker._clearDate(e.target);
						handled = !keyModifier || e[keyModifier];
						break; // clear on ctrl+end
				case 36: if (!keyModifier || e[keyModifier]) $.datepicker._gotoToday(e.target);
						handled = !keyModifier || e[keyModifier];
						break; // current on ctrl+home
				case 37: if (!keyModifier || e[keyModifier]) $.datepicker._adjustDate(e.target, -1, 'D');
						handled = !keyModifier || e[keyModifier];
						break; // -1 day on ctrl+left
				case 38: if (!keyModifier || e[keyModifier]) $.datepicker._adjustDate(e.target, -7, 'D');
						handled = !keyModifier || e[keyModifier];
						break; // -1 week on ctrl+up
				case 39: if (!keyModifier || e[keyModifier]) $.datepicker._adjustDate(e.target, +1, 'D');
						handled = !keyModifier || e[keyModifier];
						break; // +1 day on ctrl+right
				case 40: if (!keyModifier || e[keyModifier]) $.datepicker._adjustDate(e.target, +7, 'D');
						handled = !keyModifier || e[keyModifier];
						break; // +1 week on ctrl+down
				default: handled = false;
			}
		else if (e.keyCode == 36 && (!keyModifier || e[keyModifier])) // display the date picker on ctrl+home
			$.datepicker._showDatepicker(this);
		else if (e.keyCode == 40 && e.altKey) // display the date picker on alt+down	
			$.datepicker._showDatepicker(this);
		else
			handled = false;
		if (handled) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						html += '<td class="ui-datepicker-days-cell' +
							...
							(highlightCurrentDay && printDate.getTime() == selectedDate.getTime() && drawMonth == inst.selectedMonth ?
